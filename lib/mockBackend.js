...
(function (g) {
  ...
  const CATEGORIES = ["political", "ai_media", "sensitive", "misinformation"];
  ...
  function classifyOne(post) {
    ...
    else if (r < 0.22) labels.push("ai_media");
    else if (r < 0.28) labels.push("sensitive");
    else if (r < 0.32) labels.push("misinformation");

    // A rare post carries a second label, to exercise multi-category UI.
    if (r > 0.97) labels.push("ai_media", "misinformation");
  ...
  function detectText(payload) {
    ...
    if (r < 0.60) {
      ...
      verdict = "human_safe";
      label = "Human safe";
      prob = 0.01 + r * 0.48; // ~0.01–0.29
    } else if (r < 0.75) {
      band = "amber";
      verdict = "minor_ai_traces";
      label = "Minor AI traces";
      prob = 0.30 + (r - 0.60) * 0.66; // ~0.30–0.39
    } else if (r < 0.85) {
      band = "amber";
      verdict = "mixed";
      label = "Mixed signals";
      prob = 0.40 + (r - 0.75) * 1.0; // ~0.40–0.50
    } else {
      band = "red";
      verdict = "likely_ai";
      label = "This is AI";
      prob = 0.51 + (r - 0.85) * 3.2; // ~0.51–0.99
    }
    prob = Math.min(0.99, Math.round(prob * 100) / 100);

    const textExplain = {
      human_safe: "This text lacks the typical repetitive structures and perplexity patterns of AI-generated content. It reads like human writing.",
      minor_ai_traces: "Some subtle patterns detected that occasionally appear in AI writing.",
      mixed: "Some phrasing is generic or highly predictable, which can appear in both AI and human writing. Inconclusive.",
      likely_ai: "Strong structural signs of AI generation found, including highly predictable token choices and uniform sentence complexity."
    };
