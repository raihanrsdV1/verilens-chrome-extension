...
(function (g) {
  const URL_KEY = "verilens_backend_url";        // VideoVeritas (video)
  const IMAGE_URL_KEY = "verilens_image_backend_url"; // FSD (image)
  ...
  async function getImageBackendUrl() {
    const o = await chrome.storage.local.get(IMAGE_URL_KEY);
    return normalize(o[IMAGE_URL_KEY]);
  }
  ...
  // GET /health — popup "Test connection" for the image backend.
  async function pingImage(url) {
    const base = normalize(url);
    if (!base) return { ok: false, error: "No image backend URL set." };
    try {
      const r = await fetch(base + "/health", { headers: headers() });
      if (!r.ok) return { ok: false, error: "HTTP " + r.status };
      const j = await r.json().catch(() => ({}));
      return { ok: true, model: (j && j.model) || "FSD" };
    } catch (e) {
      return { ok: false, error: String((e && e.message) || e) };
    }
  }
  ...
  g.VerilensRealBackend = { getBackendUrl, getImageBackendUrl, ping, pingImage, detectVideo, detectImage };
})(globalThis);
